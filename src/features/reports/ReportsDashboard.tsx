...
import { Animal, UserRole, Shift, ClinicalNote, MARChart, InternalMovement, ExternalTransfer, MaintenanceLog, LogEntry } from '../../types';
  ...
  const generatePreview = async () => {
    ...
    try {
      ...
      if (activeReportId === 'husbandry') {
        ...
        const logs = rawLogs.map(d => d.toJSON() as LogEntry).filter(log => {
          if ((log as unknown as { is_deleted?: boolean }).is_deleted) return false;
        ...
        const sortedData = rawData.map(d => d.toJSON() as MaintenanceLog).filter(d => !d.is_deleted).sort((a, b) => new Date(a.date_logged).getTime() - new Date(b.date_logged).getTime());
        ...
        const maintenanceLogs = maintenanceLogsDocs.map(d => d.toJSON() as MaintenanceLog).filter(d => !d.is_deleted);
