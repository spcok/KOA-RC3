...
const AddEntryModal: React.FC<AddEntryModalProps> = ({ 
  ...
  const [notes, setNotes] = useState(() => {
    if (existingLog?.log_type === LogType.FEED && existingLog.notes) {
      const parsed = safeJsonParse<{ userNotes?: string }>(existingLog.notes, { userNotes: existingLog.notes });
  ...
  const [cast, setCast] = useState<'AM' | 'PM' | 'NO' | 'N/A'>(() => {
    if (existingLog?.log_type === LogType.FEED && existingLog.notes) {
      const parsed = safeJsonParse<{ cast?: 'AM' | 'PM' | 'NO' | 'N/A' }>(existingLog.notes, { cast: 'N/A' });
