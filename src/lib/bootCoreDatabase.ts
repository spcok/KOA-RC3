...
appSchemas.archived_animals = appSchemas.animals;
...
export const bootCoreDatabase = (): Promise<RxDatabase> => {
  ...
  dbPromise = (async () => {
    ...
    console.log("🟢 [Core DB] Booting Invincible RxDB Engine v16...");
    
    const db = await createRxDatabase({
      name: 'koa_manager_v16',
      storage: getRxStorageDexie()
    });

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const collectionsToAdd: Record<string, { schema: RxJsonSchema<any> }> = {};
    for (const table of SYNC_TABLES) {
      if (!db.collections[table]) {
        collectionsToAdd[table] = { schema: appSchemas[table] };
      }
    }
    ...
    console.log("✅ [Core DB] v15 Initialized and Tables Attached.");
    return db;
  })();
